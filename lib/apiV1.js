 ...
 *   GET /api/v1/search/dreps?q=          up to 10 DReps matching a name or id fragment
...
const { computeStats } = require("./governanceStats");
const calendar = require("./governanceCalendar");
...
function createApiV1(ctx) {
  ...
  function handle(url) {
    ...
    if (parts.length === 1 && parts[0] === "stats") {
      const stats = memoFor(source, "stats", () => computeStats({ ...source, committeeMembers: committeeRows(source) }));
      return { status: 200, body: { meta: meta(source, extra), stats }, cache };
    }

    if (parts.length === 1 && parts[0] === "calendar") {
      const scoped = { ...source, committeeMembers: committeeRows(source) };
      const referenceEpoch = Number(source?.latestEpoch || 0);
      // Rows at a past boundary never change for a given snapshot: memoise per epoch.
      const computeRowsAt = (epoch) => memoFor(source, `calendar-rows:${epoch}`, () =>
        model.computeActionModels(calendar.snapshotAtEpoch(scoped, epoch, referenceEpoch), { referenceEpoch: epoch }));
      const body = calendar.buildCalendar(scoped, url.searchParams.get("from"), url.searchParams.get("to"), { computeRowsAt });
      return { status: 200, body: { meta: meta(source, extra), ...body }, cache };
    }

    if (parts.length === 2 && parts[0] === "search" && parts[1] === "dreps") {
      const q = String(url.searchParams.get("q") || "").trim().toLowerCase();
      const limit = Math.min(25, Math.max(1, Number(url.searchParams.get("limit") || 10)));
      const out = [];
      if (q) {
        for (const d of Array.isArray(source?.dreps) ? source.dreps : []) {
          const id = String(d?.id || "");
          if (/always_abstain|always_no_confidence/i.test(id)) continue;
          const name = String(d?.name || d?.profile?.name || "");
          if (!id.toLowerCase().includes(q) && !name.toLowerCase().includes(q)) continue;
          out.push({ id, name, votingPowerAda: Number(d?.votingPowerAda || 0), voteCount: Array.isArray(d?.votes) ? d.votes.length : 0 });
          if (out.length >= limit) break;
        }
      }
      return { status: 200, body: { q, results: out }, cache: "public, max-age=30" };
    }
