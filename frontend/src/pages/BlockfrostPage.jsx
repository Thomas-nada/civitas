...
const GOV_ACTION_ID = "gov_action12sumv9qky4pkenmqp7gshv9nxqdk9zyn8gkd77fewkyy3t5tnxtsq457vgq";
...
export default function BlockfrostPage() {
  useSeoMeta({ title: "Blockfrost Vote Tracker", description: "Live on-chain DRep vote tracker for the Blockfrost not-for-profit treasury withdrawal." });
  ...
  const load = useCallback(async (soft = false) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/intersect?proposal=blockfrost");
  ...
  // ── Derived: predicted vote per DRep (override > on-chain > blank) ─────────
  const predictedVotes = useMemo(() => {
    if (!data) return {};
    const out = {};
    for (const d of data.dreps) out[d.id] = overrides[d.id] !== undefined ? overrides[d.id] : d.onChainVote;
    return out;
  }, [data, overrides]);
  ...
  function downloadCSV() {
    ...
    const headers = ["Rank","Name","DRep ID","Status","Counted","VP (ADA)","On-chain Vote","Predicted Vote","Has Rationale","Voted At (UTC)"];
    ...
    const csv = [
      `# Blockfrost — transformation to not-for-profit — DRep Vote Tracker (on-chain)`,
    ...
    a.href = url;
    a.download = `blockfrost-votes-${new Date(data.fetchedAt).toISOString().slice(0,10)}.csv`;
    a.click();
  ...
  const { onChainStats, totalActiveAda, fetchedAt, latestOnChainAt } = data;
    ...
    <main className="shell" style={{ paddingBottom: "4rem" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>
                Treasury Withdrawal · on-chain only (not on Ekklesia)
              </div>
      ...
      <div style={{ maxWidth: 1300, margin: "0 auto", padding: "24px 32px" }}>
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 28 }}>
          ...
          <StatCard label="On-chain Voted" value={`${onChainStats.total} DReps`} sub={`${pct(onChainStats.yesPct)} Yes on-chain`} />
        </div>

        {/* ── Panel: two-col summary ── */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 28 }}>
          {[
            { title: "On-Chain Votes (actual)", stats: onChainStats },
            { title: "Predicted (with your edits)", stats: { yesPct: liveStats.yesPct, yesAda: liveStats.yesAda, noAda: liveStats.noAda, absAda: liveStats.absAda, yesCt: liveStats.yesCt, noCt: liveStats.noCt, absCt: liveStats.absCt, denom: liveStats.denom } },
          ].map(({ title, stats }) => (
        ...
        <div style={{ background: "rgba(84,228,188,.07)", border: "1px solid rgba(84,228,188,.2)", borderRadius: 8, padding: "10px 16px", marginBottom: 20, fontSize: 13, color: "var(--text-muted)" }}>
          <strong style={{ color: "var(--mint)" }}>Interactive:</strong> Click any <em>Predicted Vote</em> pill to cycle through Yes → No → Abstain → Not Voted — including DReps who already voted on-chain, to model an expected change. Stats and bars update live. 🔒 in the On-chain column marks the actual recorded vote. This action has no Ekklesia ballot — it is on-chain only.
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16, alignItems: "center" }}>
          ...
          />
          {["All", "Yes", "No", "Abstain", "Not Voted", "Voted On-Chain"].map(f => (
        ...
        <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 14, lineHeight: 1.6 }}>
          <strong>Methodology:</strong> Threshold = 67% of active DRep VP minus abstain VP. Always-no-confidence ({fmtM(data.alwaysNoConfAda)}) counted on No side. Not-voted DReps count against Yes. Predicted starts from each DRep's on-chain vote; edit any pill to model an expected change. Top 500 DReps by voting power shown. Deregistered DReps excluded. This action was never on an Ekklesia ballot — figures are on-chain only.
        </div>
      </div>
