<!-- url: https://most-brass-sun.quicknode-ipfs.com/ipfs/QmdekGFFbC3RTThBuTRAQDUNRRmUbY1yiQFPrPnx7htTid -->
...
As propostas afirmam genericamente que o trabalho será realizado por desenvolvedores experientes em Rust e que haverá supervisão de um technical lead dedicado, cujo custo será absorvido pela própria TxPipe. Entretanto, isso não identifica a quantidade de capacidade de trabalho que está sendo efetivamente adquirida pelo Tesouro. Sem uma definição objetiva do nível esperado de dedicação, não é possível determinar se os US$ 105.000 correspondem a dez, vinte, trinta ou quase quarenta horas semanais de trabalho especializado.

Essa ausência torna a avaliação de custo-benefício excessivamente subjetiva. Não existem evidências suficientes para concluir que o preço proposto seja excessivo, inadequado ou incompatível com o mercado de desenvolvimento especializado em blockchain e Rust. Ao mesmo tempo, também não existem informações suficientes para verificar que o preço seja proporcional ao volume de trabalho contratado. A reputação da equipe e a aparente utilidade dos projetos não substituem a necessidade de explicar como o custo de trabalho foi calculado.

Os milestones trimestrais reforçam essa incerteza. Nas propostas de manutenção, os milestones repetem atividades e critérios de conclusão substancialmente semelhantes, incluindo atualizações de dependências, compatibilidade com o protocolo, correção de problemas, melhorias de desempenho, documentação, triagem de issues, revisão de contribuições e recursos para desenvolvimento compatível com inteligência artificial. Expressões como responder a issues em um “prazo razoável” não estabelecem um nível de serviço objetivo.

Também não são definidos volumes mínimos de trabalho, metas de releases, tempos de resposta, expectativas de disponibilidade, metas de resolução de problemas ou entregáveis específicos para o escopo relacionado a inteligência artificial. Algum grau de flexibilidade é adequado para trabalhos de manutenção, porque bugs futuros, alterações do protocolo e solicitações da comunidade não podem ser previstos com precisão. No entanto, essa flexibilidade deveria ser compensada por uma definição mais clara da capacidade de trabalho contratada.

A reserva de contingência e o compromisso de devolução dos recursos não utilizados são aspectos positivos. O mecanismo busca preservar um orçamento fixo em dólares caso o preço do ADA fique abaixo da taxa de conversão da proposta, exigindo a devolução ao Tesouro do ADA excedente quando não for necessário para cobrir o valor aprovado em dólares. Isso reduz o risco de que a reserva de contingência seja automaticamente convertida em remuneração adicional. Ainda assim, uma boa estrutura cambial não resolve a ausência de granularidade no orçamento principal de trabalho.

O fato de quatro propostas semelhantes serem submetidas pelo mesmo fornecedor também aumenta a importância de uma visão consolidada sobre os recursos humanos. A documentação não estabelece claramente se o trabalho financiado será realizado por quatro desenvolvedores distintos, por membros compartilhados da equipe ou por recursos parcialmente sobrepostos. Também não apresenta uma visão consolidada da capacidade total equivalente a tempo integral, das responsabilidades ou da separação do trabalho entre os quatro projetos.

Isso não demonstra que haverá cobrança sobreposta ou duplicação de trabalho. Contudo, as informações fornecidas são insuficientes para excluir esse risco ou para verificar a capacidade agregada de execução do fornecedor nos quatro contratos. Quando múltiplas solicitações simultâneas ao Tesouro utilizam descrições semelhantes de recursos part-time indefinidos, deveria ser apresentado um mapa claro de equipe e alocação.

Caso as propostas especificassem as horas esperadas, o percentual de dedicação, a senioridade, a estrutura de equipe e o cálculo dos custos de trabalho, a principal objeção orçamentária seria removida. Nessas condições, a posição mais provável seria ABSTAIN, considerando a reputação da TxPipe, seu histórico de execução e as evidências de utilização prática dos projetos, mas reconhecendo que participantes com maior qualificação técnica possuem melhores condições de determinar a essencialidade estratégica de cada ferramenta.

Nas condições atuais, contudo, uma retirada do Tesouro deve permitir que os dReps avaliem não apenas se o fornecedor possui boa reputação e se o projeto parece útil, mas também qual volume de trabalho está sendo adquirido e se o valor solicitado é proporcional a essa capacidade. Como essa informação fundamental não está disponível, a relação de custo-benefício não pode ser avaliada com confiança suficiente.

Por essas razões, o voto nas propostas de manutenção de Oura, Pallas, Dolos e UTxO RPC é NÃO.
